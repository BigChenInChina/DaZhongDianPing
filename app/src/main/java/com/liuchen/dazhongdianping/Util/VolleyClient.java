...
public class VolleyClient {
    ...
    public void getDailyDeals2(String city, final Response.Listener<TuanEntity> listener){
        //1)获取新增团购的ID列表
        ...
        /*        {
                ...
                try {
                    ...
                    if(sb.length()>0) {
                        ...
                        params2.put("deal_ids", idlist);
                        String url2 = HttpUtil.getURL("http://api.dianping.com/v1/deal/get_batch_deals_by_id", params2);
                        TuanEntityRequest req2 = new TuanEntityRequest(url2,listener);
                        queue.add(req2);
                    }else{
                        //该城市今日无新增团购
                        listener.onResponse(null);
    ...
    public class TuanEntityRequest extends Request<TuanEntity>{

        Response.Listener<TuanEntity> listener;

        public TuanEntityRequest(String url, Response.Listener<TuanEntity> listener) {
            super(Method.GET, url,null);
            this.listener = listener;
        }

        @Override
        protected Response<TuanEntity> parseNetworkResponse(NetworkResponse networkResponse) {
            String resp = new String(networkResponse.data);
            Gson gson = new Gson();
            TuanEntity tuanEntity = gson.fromJson(resp,TuanEntity.class);
            //自己组装一个Volley的Response对象作为方法的返回值

            Response<TuanEntity> result = Response.success(tuanEntity, HttpHeaderParser.parseCacheHeaders(networkResponse));

            return result;
        }
        @Override
        protected void deliverResponse(TuanEntity tuanEntity) {
            listener.onResponse(tuanEntity);
        }
    }
}
